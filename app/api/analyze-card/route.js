...
export default function BusinessCardScanner() {
  ...
  const handleFileUpload = (e) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        console.log('File read complete, image data length:', imageData?.length);
        if (imageData) {
          processImage(imageData);
        }
      };
  ...
  const processImage = async (imageData) => {
    ...
    try {
      console.log('Step 1: Image data received, length:', imageData.length);

      // Call our backend API route
      console.log('Step 2: Calling /api/analyze-card...');
      ...
      console.log('Step 4: Data parsed:', data);

      const textContent = data.content?.[0]?.text || '{}';
      console.log('Step 5: Text content extracted');
      ...
      console.log('Step 7: Contact info parsed:', contactInfo);
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '0 1rem' }}>
        <div style={{ marginBottom: '2rem' }}>
          ...
          <p style={{ margin: '0 0 2rem', color: 'var(--color-text-secondary)', fontSize: '14px' }}>
            Scan business cards with AI-powered extraction. Export to Excel or Google Sheets.
          </p>
